...
bool ImGui::ButtonExIcon(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImGui::RenderFrame(bb.Min, bb.Max, ImColor(0, 0, 0, 0), true, style.FrameRounding);

    ImVec2 text_pos = bb.Min + style.FramePadding;
    float leftGapReduction = -13.0f;
    text_pos.x += leftGapReduction;

    ImGui::RenderTextClipped(text_pos, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::ButtonIcon(const char* label, const ImVec2& size_arg)
{
    return ButtonExIcon(label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = CalcItemWidth();
    const float distance = 298.f;

    const float square_sz = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(315, 0) + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    it_nameanim->second = ImLerp(it_nameanim->second, *v ? 1.f : 0.f, g.IO.DeltaTime * 12.f);
    it_nameanim->second *= style.Alpha;

    bool is_hovered = ImGui::IsMouseHoveringRect(total_bb.Min, total_bb.Max);
    if (is_hovered)
    {
        window->DrawList->AddRectFilled(total_bb.Min + ImVec2(-5.0f, 0.0f), total_bb.Max + ImVec2(-5.0f, 0.0f), ImColor(1.0f, 1.0f, 1.0f, 0.12f));
    }

    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(4 + distance, 8), check_bb.Max + ImVec2(9 + distance, -8), ImColor(0.4f, 0.4f, 0.4f, 1.00f - it_nameanim->second), 10);
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(5 + distance, 9), check_bb.Max + ImVec2(it_nameanim->second * 11 + distance - 7, -9), ImColor(1.0f, 1.0f, 1.0f, it_nameanim->second), 10);
    float circle_center_x = check_bb.Min.x + distance + 12 + it_nameanim->second * 15;
    float circle_center_y = check_bb.Min.y + square_sz * 0.5f;  // Centered on the square

    window->DrawList->AddCircleFilled(ImVec2(circle_center_x, circle_center_y), 7.0f, ImColor(0.80f, 0.80f, 0.80f, 1.00f), 25);
    window->DrawList->AddCircle(ImVec2(circle_center_x, circle_center_y), 8.10f, ImColor(0.0f, 0.0f, 0.0f, 255.00f), 45);

    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x - 5 + style.ItemInnerSpacing.x - 30, check_bb.Min.y + style.FramePadding.y - 7.0f), label);
...
bool ImGui::CheckboxW(const char* label, bool* v, ImColor Color, float Size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y * -1, label_size.y + style.FramePadding.y * -1));
    ...
    const float pad = ImMax(1.0f, (float)(int)(check_sz / Size));

    window->DrawList->AddRect(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImColor(0, 0, 0, 255)); // Main gradient.
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImColor(255, 255, 255, 255)); // Main gradient.
    ...
    if (*v) {
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImColor(Color)); // Main gradient.
        window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.38f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.38f))); // Main gradient.
    }
    ...
    if (label_size.x > 0.0f) {
        RenderText(text_bb.Min + ImVec2(-1.0f, -3.0f), label);
    }
...
bool ImGui::CheckboxTrue(const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    window->DrawList->AddRectFilled(newRect.Min, newRect.Max, ImColor(52, 61, 70), 50);
    ...
    window->DrawList->AddRectFilled(sRect.Min, sRect.Max, ImColor(255, 255, 255), 50);

    ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);
    if (*v) {
        window->DrawList->AddRectFilled(ImVec2(newRect.Min.x - 1.f, newRect.Min.y - 1.f), ImVec2(newRect.Max.x + .5f, newRect.Max.y + 1.f), ImColor(58, 199, 62), 50);
        ...
        window->DrawList->AddRectFilled(tRect.Min, tRect.Max, ImColor(255, 255, 255), 50);
    }
    if (label_size.x > 0.0f)
...
bool ImGui::CheckboxFalse(const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 sLowerRight(check_bb.GetCenter().x, check_bb.GetBR().y + 2.f);
    ImRect sRect(sUpperLeft, sLowerRight);
    window->DrawList->AddRectFilled(sRect.Min, sRect.Max, ImColor(72, 74, 72), 50);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImGui::SetCursorPosX(w - 7.5f);

    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(117, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    // Slider behavior and value update
    ImRect grab_bb;
    ...
    // Calculate the position of the grab handle (circle) based on the current value
    float normalized_value = (*(float*)p_data - *(float*)p_min) / (*(float*)p_max - *(float*)p_min); // Normalize value to [0, 1]
    float handle_pos_x = frame_bb.Min.x + normalized_value * (frame_bb.Max.x - frame_bb.Min.x); // Position on the slider line

    const ImU32 line_col = ImColor(255, 255, 255, 255);
    const float line_thickness = 2.35f;
    window->DrawList->AddLine(frame_bb.Min + ImVec2(0, frame_bb.GetHeight() * 0.5f),
        frame_bb.Max - ImVec2(0, frame_bb.GetHeight() * 0.5f),
        line_col, line_thickness);

    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        ImVec2 slider_center = ImVec2(handle_pos_x, (grab_bb.Min.y + grab_bb.Max.y) / 2.0f);
        window->DrawList->AddCircleFilled(slider_center, 5.0f, ImColor(255, 35, 35, 255)); // Draw the filled circle
        window->DrawList->AddCircle(slider_center, 5.0f, ImColor(0, 0, 0, 255)); // Draw the circle outline
    }
    ...
    RenderTextClipped(ImVec2(total_bb.Min.x - 100, total_bb.Min.y - 25), ImVec2(total_bb.Min.x + 2, total_bb.Min.y + 56), value_buf, value_buf_end, NULL, ImVec2(0.9f, 0.5f));

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - 344, frame_bb.Min.y + style.FramePadding.y - 7.0f), label);
...
bool ImGui::SliderScalarMain(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, ImColor Color)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    {
        ImVec2 slider_center = ImVec2(handle_pos_x, (grab_bb.Min.y + grab_bb.Max.y) / 2.0f);
        window->DrawList->AddCircleFilled(slider_center, 5.0f, ImColor(Color)); // Draw the filled circle
        window->DrawList->AddCircle(slider_center, 5.0f, ImColor(0, 0, 0, 255)); // Draw the circle outline
    }
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - 340, frame_bb.Min.y + style.FramePadding.y - 7.0f), label);
...
bool ImGui::SliderFloatMain(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags, ImColor Color)
{
    return SliderScalarMain(label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags, Color);
}
